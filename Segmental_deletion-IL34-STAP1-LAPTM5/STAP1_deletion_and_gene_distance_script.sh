...
cd CDS_sorted
ls -1 *.bed|cut -f1 -d'.'|sort -u > /media/lokdeep/sdf/BUDDHA/BMCBIO/STAP1_deletion/CDS_filtered/"$gene".species.lst
cd /media/lokdeep/sdf/BUDDHA/BMCBIO/STAP1_deletion
done
...
do
cp /media/lokdeep/sdf/BUDDHA/BMCBIO/STAP1_deletion/$gene/ncbi_dataset/data/CDS_sorted/"$species_name"."$gene".bed $gene
done
echo $gene
done
