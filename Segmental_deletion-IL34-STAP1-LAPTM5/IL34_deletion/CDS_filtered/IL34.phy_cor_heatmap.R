...
getwd()
current_dir <- getwd()
gene_name <- sub(".*/Segmental_deletion/([^/_]+)_deletion.*", "\\1", current_dir)
print(gene_name)
library(ggplot2)
library(reshape2)
library(pheatmap)
...
sum(p_values_adj_matrix[ , 4] < 0.05, na.rm = TRUE)
...
mtext(bquote("Relative distance (in %): " ~ 
      ...
      side = 1, line = 4, at = 50)
mtext(bquote(italic(.(gene_name)) ~ "gene status"), side = 2, line = 2, at = 0.5)
...
# Count p-values < 0.05 in 4th column, ignoring NAs
sig_count <- sum(p_values_adj_matrix[ , 4] < 0.05, na.rm = TRUE)
print(sig_count)
# Print specific values
print(ig10pvals[[1]])
print(mplepvals[[1]])
loss <-table(df$Gene_status)[[1]]
intact <-table(df$Gene_status)[[2]]
total_species <- length(df$Gene_status)
# Save values to TSV file
output_df <- data.frame(
  Gene = gene_name,
  Squamates = table(df$Gene_status)[[1]],
  Non_Squamates = table(df$Gene_status)[[2]],
  Total_Species = length(df$Gene_status),
  Sig_Pvals_Count = sig_count,
  IG10_Pval = ig10pvals[[1]],
  MPLE_Pval = mplepvals[[1]]
)

# Write to file
write.table(output_df, file = paste0(gene_name, "_pvals_summary.tsv"),
            sep = "\t", quote = FALSE, row.names = FALSE)

# Confirm
print(paste("Results saved to", paste0(gene_name, "_pvals_summary.tsv")))
