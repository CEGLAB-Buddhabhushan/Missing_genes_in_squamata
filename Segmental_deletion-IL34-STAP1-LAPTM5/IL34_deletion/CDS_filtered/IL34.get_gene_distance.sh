...
cd IL34_deletion
mkdir CDS_filtered
for i in COG4-25839 SF3B3-23450 MTSS2-92154 VAC14-55697
do
...
cd CDS_sorted
ls -1 *.bed|cut -f1 -d'.'|sort -u > /mnt/disk4/BUDDHA/BMC-Bio/IL34_deletion/CDS_filtered/"$gene".species.lst
cd /mnt/disk4/BUDDHA/BMC-Bio/IL34_deletion
done

# get common species which have annoated in all genes
#COG4-25839 SF3B3-23450 MTSS2-92154 VAC14-55697
cd CDS_filtered
comm -12 <(sort -u COG4.species.lst) <(sort -u SF3B3.species.lst) | comm -12 - <(sort -u MTSS2.species.lst) | comm -12 - <(sort -u VAC14.species.lst)  > common.species.lst 
for gene in COG4 SF3B3 MTSS2 VAC14
do
mkdir $gene
for species_name in `cat common.species.lst`
do
cp /mnt/disk4/BUDDHA/BMC-Bio/IL34_deletion/$gene/ncbi_dataset/data/CDS_sorted/"$species_name"."$gene".bed $gene
done
echo $gene
done
