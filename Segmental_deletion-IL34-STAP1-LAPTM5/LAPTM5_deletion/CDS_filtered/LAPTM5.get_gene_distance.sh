...
rm -r LAPTM5_deletion
mkdir  LAPTM5_deletion
cd LAPTM5_deletion
mkdir CDS_filtered
for i in PUM1-9698 SDC3-9672 MATN1-4146 PTPRU-10076
do
...
cd CDS_sorted
ls -1 *.bed|cut -f1 -d'.'|sort -u > /mnt/disk4/BUDDHA/BMC-Bio/LAPTM5_deletion/CDS_filtered/"$gene".species.lst
cd /mnt/disk4/BUDDHA/BMC-Bio/LAPTM5_deletion
done

# get common species which have annoated in all genes
#PUM1-9698 SDC3-9672 MATN1-4146 PTPRU-10076
cd CDS_filtered
comm -12 <(sort -u PUM1.species.lst) <(sort -u SDC3.species.lst) | comm -12 - <(sort -u MATN1.species.lst) | comm -12 - <(sort -u PTPRU.species.lst)  > common.species.lst 
for gene in PUM1 SDC3 MATN1 PTPRU
do
mkdir $gene
for species_name in `cat common.species.lst`
do
cp /mnt/disk4/BUDDHA/BMC-Bio/LAPTM5_deletion/$gene/ncbi_dataset/data/CDS_sorted/"$species_name"."$gene".bed $gene
done
echo $gene
done

##### Get gene distance table
genes=("PUM1" "SDC3" "MATN1" "PTPRU")
...
for i in $(cat species_list.txt); do
    # Merge BED regions
    merged_region=$(cat PUM1/"$i".PUM1.bed PTPRU/"$i".PTPRU.bed | \
                    sort -k1,1 -k2,2n | \
    ...
    efetch -db nuccore -id $id -format fasta -chr_start $chr_start -chr_stop $chr_stop | \
        gzip > "$i".PUM1_PTPRU.fasta.gz

    klumpy find_gaps --fasta "$i".PUM1_PTPRU.fasta.gz 

    # Check if the gaps file exists, else set num_gaps = 0
    if [[ -f "$i".PUM1_PTPRU_gaps.tsv ]]; then
        num_gaps=$(tail -n +2 "$i".PUM1_PTPRU_gaps.tsv | wc -l)
    else
        num_gaps=0
    fi
    ...
    echo -e "$i\t$num_gaps"
    rm "$i".PUM1_PTPRU.fasta.gz  "$i".PUM1_PTPRU_gaps.tsv
done
